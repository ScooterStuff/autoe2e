...
class CandidateActionExtractor:
    # Dropdown toggle indicators - actions with these are extracted but flagged
    DROPDOWN_INDICATORS = [
        'dropdown-toggle',
    ...
    @staticmethod
    def is_dropdown_toggle_element(element: Element) -> bool:
        """Check if an element is a dropdown toggle based on its HTML."""
        outer_html = element.outerHTML.lower()
        for indicator in CandidateActionExtractor.DROPDOWN_INDICATORS:
            if indicator in outer_html:
                return True
        return False
    ...
    def extract_click_actions(driver: WebDriver) -> list[Action]:
        ...
        # Filter out ephemeral popup elements (datepickers, calendars, modals, etc.)
        visible_elements = list(filter(lambda x: not CandidateActionExtractor.is_ephemeral_element(x), visible_elements))
        actions: list[Action] = list(map(ClickAction, visible_elements))
        return actions
    
    
    @staticmethod
    def is_ephemeral_element(element: Element) -> bool:
        """
        Check if an element is part of an ephemeral popup (datepicker, calendar, modal overlay).
        These elements should not be captured as separate actions because:
        1. They disappear when the popup closes
        2. They're part of form input, not navigation
        3. They can't be reliably re-clicked after state reload
        """
        outer_html = element.outerHTML.lower()
        xpath = element.get_id().lower() if element.get_id() else ""
        
        # Datepicker/Calendar indicators
        ephemeral_patterns = [
            'mat-datepicker',
            'mat-calendar',
            'datepicker',
        ...
        for pattern in ephemeral_patterns:
            if pattern in outer_html or pattern in xpath:
                return True
